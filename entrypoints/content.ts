...
export default defineContentScript({ matches: ['<all_urls>'], runAt: 'document_idle' }, () => {
  let host: HTMLDivElement | undefined;
  const close = () => { host?.remove(); host = undefined; };
  const selectionData = () => {
  ...
  const open = () => {
    ...
    close(); host = document.createElement('div'); host.id = 'keep-sentence-capture';
  ...
  browser.runtime.onMessage.addListener((message) => { if (message.type === 'open-capture') open(); });
});
