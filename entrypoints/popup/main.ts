...
async function render() {
  ...
  app.innerHTML = `<main><header><span class="mark">///</span><strong>Keep the Sentence</strong><p>${vault.records.length} saved locally</p></header>${next ? `<section class="review" aria-labelledby="review-title"><p class="label">Review in context</p><h1 id="review-title">${escape(next.phrase)}</h1><blockquote>${escape(next.context)}</blockquote><details><summary>Show your meaning</summary><p>${escape(next.gloss)}</p></details><button id="review">I remembered it</button></section>` : `<section class="empty"><h1>No phrases due today</h1><p>Select a phrase on any page. Choose “Keep this sentence”.</p></section>`}<section class="list"><div><h2>Saved phrases</h2><button class="link" id="export">Export CSV</button></div>${vault.records.slice(0, 8).map((r) => `<article><strong>${escape(r.phrase)}</strong><span>${escape(r.title)}</span><small>${r.reviews} review${r.reviews === 1 ? '' : 's'}</small></article>`).join('') || '<p>Your saved phrases will appear here.</p>'}</section></main>`;
